...
const char *WIFI_PASSWORD = "helmet12";
    ...
    float accelChange)
{
  ...
  json += "\"lastUpdatedMs\":" + String(nowMs) + ",";
  json += "\"lastUpdated\":\"" + jsonEscape(gpsUtcIso.length() ? gpsUtcIso : String(nowMs)) + "\"";
  json += "},";

  json += "\"location\":{";
  json += "\"lat\":" + String(lat, 6) + ",";
  json += "\"lng\":" + String(lng, 6) + ",";
  json += "\"latLng\":\"" + jsonEscape(latLng) + "\",";
  json += "\"googleMapsUrl\":\"" + jsonEscape(googleMapsUrl) + "\",";
  ...
  // ===== LIVE STATE UPDATE =====
  HTTPClient httpLive;
  String liveUrl = String(FIREBASE_BASE_URL) + "/helmets/" + HELMET_ID + ".json";
  ...
  Serial.println("----- LIVE HELMET DATA UPDATE -----");
  Serial.println(liveUrl);
  Serial.println(json);
  Serial.print("LIVE HTTP Code: ");
  Serial.println(liveCode);

  if (liveCode > 0)
  {
    Serial.println("LIVE Response: " + httpLive.getString());
  }
  else
  {
    Serial.println("LIVE update failed.");
  }
  ...
  // ===== LOG ENTRY UPDATE =====
  HTTPClient httpLog;
  String logUrl = String(FIREBASE_BASE_URL) + "/helmet_logs/" + HELMET_ID + "/" + String(nowMs) + ".json";

  httpLog.begin(logUrl);
  httpLog.addHeader("Content-Type", "application/json");

  int logCode = httpLog.PUT(json);

  Serial.println("----- HELMET LOG UPDATE -----");
  Serial.println(logUrl);
  Serial.print("LOG HTTP Code: ");
  Serial.println(logCode);

  if (logCode > 0)
  {
    Serial.println("LOG Response: " + httpLog.getString());
  }
  else
  {
